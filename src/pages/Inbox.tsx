...
export default function Inbox() {
  ...
  if (authLoading || loading) return (
    <div className="page" style={{ display: 'block', textAlign: 'center', padding: '4rem' }}>{t('inbox.title')}</div>
  );

  if (!user) return (
    <div className="page" style={{ display: 'block', textAlign: 'center', padding: '6rem 2rem' }}>
      <div style={{ marginBottom: '1.5rem', color: 'var(--muted)' }}>{t('inbox.loginRequired')}</div>
      <button className="btn-primary" style={{ width: 'auto', padding: '0.75rem 2.5rem' }} onClick={openAuthModal}>{t('inbox.login')}</button>
    </div>
  );

  const listingMap = new Map<string, Conversation['listing']>(conversations.map(c => [c.listingId, c.listing]));
  const uniqueListings = Array.from(listingMap.entries()).map(([id, listing]) => ({ id, title: listing?.title || t('inbox.listing') }));
    ...
    <div className="page" style={{ display: 'block' }}>
      <div className="inbox-layout">
        <div className="inbox-header">
          <div className="inbox-title">{t('inbox.title')}</div>
        </div>
        ...
        {visible.length === 0 ? (
          <div className="empty">
            ...
            <div className="empty-title">{t('inbox.noMessages')}</div>
            <div className="empty-text">{t('inbox.noMessagesText')}</div>
          </div>
        ) : (
          <div className="inbox-list">
            {visible.map(conv => {
              ...
              const otherName = conv.otherProfile?.username ? `@${conv.otherProfile.username}` : t('inbox.user');
              return (
                <Link
                ...
                >
                  ...
                  <div className="inbox-row-body">
                    ...
                    <div className="inbox-row-listing">{conv.listing?.title || t('inbox.listing')}</div>
